...
const cleanQ=(s:string)=>String(s||'').replace(/^\s*(?:답변\s*필수\s*)?\d+\s*[.)、:\-]?\s*/i,'').trim();
...
function choose(root:Element,a:string,checked=true){
 const n=norm(a),all=[...root.querySelectorAll('label,[role="option"],[role="radio"],[role="checkbox"],option')];
 const el=all.find(x=>norm(textOf(x))===n)||all.find(x=>norm(textOf(x)).includes(n));if(!el)return false;
...
function clickButton(label:string){const n=norm(label),els=[...document.querySelectorAll('button,[role="button"],input[type="submit"],input[type="button"],a')].filter(vis),el=els.find(x=>norm(textOf(x))===n)||els.find(x=>norm(textOf(x)).includes(n));if(!el)return false;(el as HTMLElement).click();return true}
async function runForm(form:any,p:any,onAdvance?:()=>Promise<void>){
 for(const s of form.steps||[]){
  if(s.type==='action'){const target=tpl(s.text||s.answer,p);if(!target)return{ok:false,message:'실행할 버튼 이름이 없습니다.'};if(onAdvance)await onAdvance();const ok=clickButton(target);if(!ok)return{ok:false,message:'버튼을 찾지 못했습니다: '+target};if(s.nextUrl)setTimeout(()=>{location.href=tpl(s.nextUrl,p)},100);return{ok:true,message:'다음 동작을 실행했습니다.',advanced:true}}
  const r=find(s.question||'');if(!r)return{ok:false,message:'문항을 찾지 못했습니다: '+s.question};
  const rawAnswer=s.answer,values=Array.isArray(rawAnswer)?rawAnswer.map((v:any)=>tpl(v,p)):tpl(rawAnswer,p),a=Array.isArray(values)?values[0]||'':values;
...
async function executePending(p:any){
 const i=Math.max(0,p?.cursor||0),forms=p?.kit?.forms||[],form=forms[i];if(!form||!matches(tpl(form.url,p)))return;
 const next={...p,cursor:i+1},result=await runForm(form,p,async()=>{await chrome.storage.session.set({pendingRun:next})});
 if(!result.advanced)await chrome.storage.session.remove('pendingRun');
...
window.addEventListener('message',e=>{if(e.source!==window||e.data?.source!=='form-routine-kit-web')return;if(e.data.type==='prepare'){chrome.storage.session.set({pendingRun:{...e.data.payload,cursor:0}}).then(()=>window.postMessage({source:'form-routine-kit-extension',type:'status',message:'실행 정보를 저장했습니다.'},'*')).catch(()=>{})}if(e.data.type==='analyze-current-tab')chrome.runtime.sendMessage({cmd:'web-analyze'}).catch(()=>{});if(e.data.type==='get-analysis')chrome.runtime.sendMessage({cmd:'get-analysis'}).then(a=>window.postMessage({source:'form-routine-kit-extension',type:'analysis',analysis:a},'*')).catch(()=>{})});
