...
const notice=[
 ...
 ['4. 사이트별 권한','확장 프로그램은 모든 웹사이트에 대한 권한을 미리 갖지 않습니다. Kit 실행 시 열리는 대상 사이트에 대해서만 Chrome이 사이트 접근 권한을 요청하며, 이용자가 직접 허용한 뒤에만 자동 입력을 시작합니다.'],
 ['5. 실행 범위','확장 프로그램은 Kit에 지정된 URL을 새 탭으로 열고, 이용자가 접근을 허용한 해당 사이트에서만 정해진 답변 입력 기능을 실행합니다. 개인정보를 별도의 개발자 서버로 전송하지 않습니다.'],
...
export default function App(){
 ...
 const prepare=()=>new Promise<boolean>(resolve=>{const id='p-'+Date.now();let done=false;const cleanup=()=>{clearTimeout(to);window.removeEventListener('message',on)};const on=(e:MessageEvent)=>{if(e.source!==window||e.origin!==location.origin||e.data?.source!=='form-routine-kit-extension'||e.data.requestId!==id)return;if(e.data.type==='prepared'){done=true;cleanup();resolve(true)}if(e.data.type==='prepare-error'){done=true;cleanup();setStatus(e.data.message||'실행 준비에 실패했습니다.');resolve(false)}};const to=window.setTimeout(()=>{if(!done){done=true;cleanup();setStatus('확장 프로그램 응답 시간이 초과되었습니다. 확장 프로그램을 새로고침한 뒤 다시 시도해 주세요.');resolve(false)}},120000);window.addEventListener('message',on);send('prepare',{requestId:id,payload:window.__formRoutinePayload})});
