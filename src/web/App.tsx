...
type A={url:string;questions:Q[];buttons:string[]};
...
export default function App(){
 const[mode,setMode]=useState<'run'|'build'>('run'),[kits,setKits]=useState<Kit[]>([]),[file,setFile]=useState(''),[name,setName]=useState(''),[num,setNum]=useState(''),[agree,setAgree]=useState(false),[analysis,setAnalysis]=useState<A|null>(null),[urlTemplate,setUrlTemplate]=useState(''),[answers,setAnswers]=useState<Record<number,string>>({}),[status,setStatus]=useState('');
 ...
 useEffect(()=>{
   ...
   const f=(e:MessageEvent)=>{if(e.source!==window||e.data?.source!=='form-routine-kit-extension')return;if(e.data.type==='analysis'){setAnalysis(e.data.analysis);setUrlTemplate(e.data.analysis?.url||'')}if(e.data.type==='status')setStatus(e.data.message)};
 ...
 const analyze=()=>{setStatus('현재 활성 탭의 폼을 읽습니다.');send({type:'analyze-current-tab'})};
 ...
 const save=()=>{
   ...
   const kit={version:2,title:'새 Form Routine Kit',forms:[{url:urlTemplate.trim()||analysis.url,steps}]};
