...
const notice=[
 ...
 ['4. 확장 프로그램 권한','설치 시 모든 사이트에 접근 권한을 부여하지 않습니다. 대상 페이지가 실제로 로드된 뒤 해당 사이트 출처에 대해서만 접근 권한을 요청하고, 허용된 경우에만 페이지를 읽습니다.'],
 ['5. 권한 제거','정해진 답변 입력이 끝나면 해당 사이트의 호스트 권한을 제거하고 작업을 종료합니다.'],
...
export default function App(){
 const[kits,setKits]=useState<Kit[]>([]),[locals,setLocals]=useState<LocalKit[]>([]),[file,setFile]=useState(''),[date,setDate]=useState(''),[checked,setChecked]=useState(false),[status,setStatus]=useState('');
 ...
 return <main className="shell">
  ...
  <section className="card"><h1>이벤트 응모 도우미</h1><p className="muted">개인정보를 받지 않고, Kit의 정해진 답변만 입력합니다.</p><button className="noticeAgain" onClick={()=>{setNoticeOpen(true);setNoticeEnd(false)}}>개인정보·권한 안내 다시 보기</button>
   ...
   <button className="primary" disabled={!checked||!file} onClick={run}>Kit 실행</button><a className="download" href="./download/form-routine-kit-extension.zip" download>Chrome 확장 프로그램 ZIP 다운로드</a><p className="status">{status}</p>
  </section>
 </main>;
}
